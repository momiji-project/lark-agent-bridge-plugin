...
4. Choose the Lark CLI identity independently from filesystem permissions.
   ...
   - Minutes and personal documents require `user-default`. Explain the additional reach and obtain explicit approval first. Before changing the preset, complete the same profile-private QR device flow defined by the standard `lark-setup` entry: start with `auth login --domain minutes --domain docs --no-wait --json`, show the exact URL plus PNG QR, end the turn, then after the user confirms run `auth login --device-code` yourself and verify the required scopes. Only then add `--lark-cli-identity user-default --confirm-user-default`.
   - Do not offer App Secret entry or another OAuth method as a normal alternative. If the QR is unreadable, use its unchanged verification URL as the fallback.
