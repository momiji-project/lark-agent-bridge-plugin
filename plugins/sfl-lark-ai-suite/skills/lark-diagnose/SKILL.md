---
name: lark-diagnose
description: 統合Plugin導入後のMacまたはWindowsで、Lark接続環境をインストール・更新・設定変更・起動・再起動せず再診断する互換入口。OS/CPU、Node.js/npm、Codex/Claude Code、Lark公式CLI、lark-channel-bridge、既存プロファイルと常駐状態を確認する。Plugin導入前の初回診断には使わない。
---

# Lark接続環境の診断だけを行う

これは基盤導入ではない。統合Pluginを導入済みの環境で使う、独立した読取専用の再診断である。

新規利用者の初回診断は、このSkillを使うためにPluginを先に追加してはならない。公開導入ガイドに掲載したWindows PowerShell用またはmacOS Terminal用の独立した読取専用診断コマンドで先に診断し、不足が見つかって導入を進める場合だけ分離版Pluginを追加する。初回診断に `codex exec --sandbox read-only` は使わない。
