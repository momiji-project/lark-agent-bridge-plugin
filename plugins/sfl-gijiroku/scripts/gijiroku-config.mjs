...
function paths(profile) {
  const override = process.env.SFL_GIJIROKU_HOME || process.env.GIJIROKU_IMAGE_BRIDGE_HOME;
  let applicationRoot;
  if (override) {
    applicationRoot = resolve(override);
  } else if (process.platform === "win32") {
    applicationRoot = join(process.env.LOCALAPPDATA || join(homedir(), "AppData", "Local"), APP_NAME);
  } else if (process.platform === "darwin") {
    applicationRoot = join(homedir(), "Library", "Application Support", APP_NAME);
  } else {
    applicationRoot = join(process.env.XDG_CONFIG_HOME || join(homedir(), ".config"), APP_NAME);
  }
