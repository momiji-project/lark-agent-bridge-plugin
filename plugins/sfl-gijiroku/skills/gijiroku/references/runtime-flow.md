...
作業ディレクトリは同期ドライブ外に作る。OSごとの既定ルートは次の通り。

```text
macOS:   ~/Library/Application Support/sfl-gijiroku/profiles/<profile>/runs/<request-id>/
Windows: %LOCALAPPDATA%\sfl-gijiroku\profiles\<profile>\runs\<request-id>\
Linux:   ${XDG_CONFIG_HOME:-~/.config}/sfl-gijiroku/profiles/<profile>/runs/<request-id>/
```
